    ...
    safeParse: function(value: string): ValidationResult {
      ...
      if (!value || value.trim() === '') {
        ...
        if (this._minLength && value.length < this._minLength) {
          errors.push(this._minMessage || `Must be at least ${this._minLength} characters`);
        }
      }
      
      return {
        success: errors.length === 0,
        error: errors.length > 0 ? { issues: errors.map(msg => ({ message: msg })) } : null,
...
export const loginSchema = {
  username: z.string().min(1,'please insert your email or phone number').userName('please enter a valid username'),
...
export const signupSchema = {
  ...
  password: z.string().min(6, "Password must be at least 6 characters long"),
  phoneNumber: z.string().phone("Please enter a valid phone number")
};
